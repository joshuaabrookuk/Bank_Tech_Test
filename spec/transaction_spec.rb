...
describe Transaction do
  ...
  it 'print method should print header' do
    ...
    expect(transaction.single_transaction[0]).to eq 'date || credit || debit || balance'
  end
  ...
  it 'print method should show transaction' do
    ...
    transaction.deposit(1000)
    transaction.print_out
    expect(transaction.single_transaction[1]).to eq "#{transaction.created_at} || 1000.00 || || 1000.00"
  end
  ...
  it 'print method should show most recent transcation first' do
    ...
    transaction.deposit(2000)
    transaction.print_out
    expect(transaction.single_transaction[1]).to eq "#{transaction.created_at} || 2000.00 || || 3000.00"
  end
  ...
  it 'print method should have correctly formatted withdrawal method' do
    ...
    transaction.withdrawal(500)
    transaction.print_out
    expect(transaction.single_transaction[1]).to eq "#{transaction.created_at} || || 500.00 || 2500.00"
  end
end
